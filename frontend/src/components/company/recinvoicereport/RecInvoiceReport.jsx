...
function RecInvoiceReport() {
  ...
  const fetchRecInvoiceReportDetails = () => {
    axios
      .get(`${config.base_url}/fetch_rec_invoice_report_details/${ID}/`)
      .then((res) => {
  ...
  const handleCustomize = (e) => {
    ...
    axios
      .post(
        `${config.base_url}/fetch_rec_invoice_report_details_customized/`,
        det
      )
      .then((res) => {
        console.log("REPRT DATA=", res);
  ...
  function reportPDF() {
    ...
    var opt = {
      ...
      filename: "Report_RecInvoice_Details" + date,
  ...
  function ExportToExcel() {
    ...
    XLSX.writeFile(wb, "Report__RecInvoice_Details" + date + ".xlsx");
  }
    ...
    if ((st != "" && end == "") || (st == "" && end != "")) {
      ...
      if (emailsString === "") {
        ...
        if (invalidEmails.length > 0) {
          ...
          axios
            .post(
              `${config.base_url}/share_rec_invoice_report_details_email/`,
              em
            )
            .then((res) => {
              if (res.data.status) {
                          ...
                          >
                            <thead>
                              <tr>
                                ...
                                <th className="text-center">REC.INVOICE NO.</th>
                                <th className="text-center">ORDER NO.</th>
                                <th className="text-center">
                                  CUSTOMER NAME
                                  <i
                                    className="fas fa-solid fa-caret-up arrow"
                                    onClick={() => sortName(5)}
                                  ></i>
                                </th>
                                <th className="text-center">INVOICE AMOUNT</th>
                                <th className="text-center">BALANCE DUE</th>
                              </tr>
                            </thead>
                            <tbody>
                              {reportData.length > 0 &&
...
export default RecInvoiceReport;
