...
function AddDebitNote() {
    ...
    axios
      ...
      .then((res) => {
        if (res.data.status) {
          ...
          setNextDebitNoteNo(res.data.dnNo);
        }
      })
      .catch((err) => {
        console.log(err);
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post(`${config.base_url}/create_new_debit_note/`, formData)
      .then((res) => {
  ...
  function getBillNumbers(val) {
    ...
    if (vendorId != "") {
      if (billType != "") {
        ...
        axios
          .get(`${config.base_url}/get_bill_numbers/`, { params: i })
          .then((res) => {
            if (res.data.status) {
        ...
        <div className="card radius-15 h-20">
          <div className="row">
            <div className="col-md-12">
              <center>
                <h2 className="mt-3">NEW DEBIT NOTE</h2>
              </center>
              <hr />
            </div>
          </div>
