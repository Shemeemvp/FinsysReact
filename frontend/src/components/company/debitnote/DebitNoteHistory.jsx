...
function DebitNoteHistory() {
  ...
  const fetchDebitNoteHistory = () => {
    axios
      .get(`${config.base_url}/fetch_debit_note_history/${debitNoteId}/`)
      .then((res) => {
        if (res.data.status) {
          var sls = res.data.debitNote;
    ...
    <>
      ...
      >
        <Link
          ...
          to={`/view_debit_note/${debitNoteId}/`}
        >
          <i
            ...
            <div id="history">
              <center>
                <h3 className="mt-3 text-uppercase">
                  #{debitNote.debit_note_no} - TRANSACTIONS
                </h3>
              </center>
