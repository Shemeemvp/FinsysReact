...
function RecInvoice() {
  ...
  function exportToExcel() {
    const Table = document.getElementById("recInvoiceTable");
    const ws = XLSX.utils.table_to_sheet(Table);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Sheet1");
    XLSX.writeFile(wb, "Recurring_Invoices.xlsx");
  }

  function sortTable(columnIndex) {
    var table, rows, switching, i, x, y, shouldSwitch;
    table = document.getElementById("recInvoiceTable");
    switching = true;
  ...
  function filterTable(row,filterValue) {
    var table1 = document.getElementById("recInvoiceTable");
  ...
  function searchTable(){
    var rows = document.querySelectorAll('#recInvoiceTable tbody tr');
  ...
  const fetchInvoice = () =>{
    axios.get(`${config.base_url}/fetch_rec_invoices/${ID}/`).then((res)=>{
      console.log("RINV RES=",res)
      if(res.data.status){
        var inv = res.data.recInvoice;
        setRecInvoice([])
        inv.map((i)=>{
          setRecInvoice((prevState)=>[
            ...prevState, i
          ])
        })
      }
        ...
        <div className="card radius-15 h-20">
          <div className="row">
            <div className="col-md-12">
              <center>
                <h2 className="mt-3">RECURRING INVOICES</h2>
              </center>
              <hr />
            </div>
          </div>
        ...
        <div className="card radius-15">
            ...
            <div className="container-fluid">
              <div className="row">
                ...
                <div className="col-md-6 d-flex justify-content-end">
                  ...
                  <Link to="/add_rec_invoice" className="ml-1">
                    <button
                      type="button"
                      style={{ width: "fit-content", height: "fit-content" }}
                      className="btn btn-outline-secondary text-grey"
                    >
                      <i className="fa fa-plus font-weight-light"></i> Rec. Invoice
                    </button>
                  </Link>
                </div>
              </div>
          ...
          <div className="table-responsive">
            ...
            >
              ...
              <tbody>
                {recInvoice &&recInvoice.map((i,index)=>(
                  <tr
                    className="clickable-row"
                    onClick={()=>navigate(`/view_rec_invoice/${i.id}/`)}
                    style={{ cursor: "pointer" }}
                  >
                    <td>{index+1}</td>
                    <td>{i.rec_invoice_date}</td>
                    <td>{i.rec_invoice_no}</td>
