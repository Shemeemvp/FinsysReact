...
function AddRetInvoice() {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (res.data.status) {
          ...
          setNextRetInvoiceNo(res.data.invNo);
        }
      })
      .catch((err) => {
        console.log(err);
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post(`${config.base_url}/create_new_ret_invoice/`, formData)
      .then((res) => {
        console.log("RINV RES=", res);
        if (res.data.status) {
          Toast.fire({
            icon: "success",
            title: "Ret Invoice Created",
          });
        ...
        <div className="card radius-15 h-20">
          <div className="row">
            <div className="col-md-12">
              <center>
                <h2 className="mt-3">NEW RETAINER INVOICE</h2>
              </center>
              <hr />
            </div>
          </div>
