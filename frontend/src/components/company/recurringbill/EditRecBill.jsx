...
function EditRecBill() {
  ...
  const fetchRecBillData = () => {
    axios
      .get(`${config.base_url}/fetch_rec_bill_data/${ID}/`)
      .then((res) => {
    ...
    axios
      ...
      .then((res) => {
        if (res.data.status) {
          ...
          setVendName(bill.vendor_name);
          setVendEmail(bill.vendor_email);
          setVendGstType(bill.vendor_gst_type);
          setVendGstIn(bill.vendor_gstin);
          setVendBillingAddress(bill.vendor_billing_address);
          setVendPlaceOfSupply(bill.vendor_place_of_supply);
          setCustomer(bill.Customer);
          setCustName(bill.customer_name);
          setCustEmail(bill.customer_email);
          setCustGstType(bill.customer_gst_type);
          setCustGstIn(bill.customer_gstin);
          setCustBillingAddress(bill.customer_billing_address);
          setCustPlaceOfSupply(bill.customer_place_of_supply);
          setProfileName(bill.profile_name);
          setRefNo(bill.reference_no);
          setRecBillNo(bill.rec_bill_no);
          setPurchaseOrderNo(bill.purchase_order_no);
          setDate(bill.date);
          setEndDate(bill.exp_ship_date);
          setTerm(bill.payment_terms);
          setRepeatEvery(bill.repeat_every);
          setPaymentMethod(bill.payment_method);
          setChequeNumber(bill.cheque_no);
          setUpiId(bill.upi_no);
          setAccountNumber(bill.bank_acc_no);
          setPriceList(bill.price_list_applied);
          setPriceListId(bill.price_list);
          ...
          setShippingCharge(bill.shipping_charge);
          setGrandTotal(bill.grandtotal);
          setPaid(bill.paid);
          setBalance(bill.balance);
          setDescription(bill.note);
          setBillItems([]);
  ...
  const [recBillNo, setRecBillNo] = useState("");
  const [purchaseOrderNo, setPurchaseOrderNo] = useState("");
  const [profileName, setProfileName] = useState("");
  const [nextRecBillNo, setNextRecBillNo] = useState("");
  ...
  async function applyPriceList(priceListId) {
    if (priceListId === "") {
      ...
      let updatedItems = await Promise.all(
        billItems.map(async (pItem) => {
          var itemId = pItem.item;
          var plc = vendPlaceOfSupply;
  ...
  const handleSubmit = (e) => {
    ...
    formData.append("rec_bill_no", recBillNo);
    ...
    formData.append("exp_ship_date", endDate);
    ...
    axios
      .put(`${config.base_url}/update_rec_bill/`, formData)
      .then((res) => {
        if (res.data.status) {
          Toast.fire({
            icon: "success",
            title: "Rec Bill Updated",
          });
          navigate(`/view_rec_bill/${billId}/`);
        }
  ...
  function handleBillNoChange(val) {
    setRecBillNo(val);
    checkBillNo(val);
  }

  function checkBillNo(val) {
    document.getElementById("BILLNoErr").innerText = "";
    var bill_num = val;
    if (bill_num != "") {
      var s = {
        Id: ID,
      ...
      axios
        .get(`${config.base_url}/check_rec_bill_no/`, { params: s })
        .then((res) => {
          console.log("INV NUM Res=", res);
          if (!res.data.status) {
            document.getElementById("BILLNoErr").innerText = res.data.message;
          } else {
            document.getElementById("BILLNoErr").innerText = "";
          }
        })
  ...
  const handleBillItemsInputChange = (id, field, value) => {
    setBillItems((prevItems) =>
      prevItems.map((item) =>
  ...
  const handleItemChange = (value, id) => {
    ...
    if (!exists) {
      if (vendPlaceOfSupply != "") {
        handleBillItemsInputChange(id, "item", value);
      ...
      alert(
        "Item already exists in the Bill, choose another or change quantity.!"
      );
    }
  };

  const handleQtyChange = (value, id) => {
    handleBillItemsInputChange(id, "quantity", value);
  ...
  function getItemData(item, id) {
    ...
    if (!exists) {
      if (plc != "") {
        if (priceList && PLId == "") {
          handleBillItemsInputChange(id, "item", "");
  ...
  function calc2(vendPlaceOfSupply) {
    const updatedItems = billItems.map((item) => {
    ...
    setBillItems(updatedItems);
    refreshIndexes(updatedItems);
    calc_total2(updatedItems, vendPlaceOfSupply);
  }
  ...
  function calc_total(billItems) {
    ...
    splitTax(taxamount, vendPlaceOfSupply);
  }

  function splitTax(taxamount, vendPlaceOfSupply) {
    var d = 0;
    if (vendPlaceOfSupply == cmpState) {
      var gst = taxamount / 2;
  ...
  function calc_total2(billItems, vendPlaceOfSupply) {
    var total = 0;
    var taxamount = 0;
    billItems.map((item) => {
      total += parseFloat(item.total || 0);
    ...
    var bal = gtot - adv_val;
    setBalance(bal.toFixed(2));
    splitTax2(taxamount, vendPlaceOfSupply);
  }

  function splitTax2(taxamount, vendPlaceOfSupply) {
    var d = 0;
    if (vendPlaceOfSupply == cmpState) {
      var gst = taxamount / 2;
      setCgst(parseFloat(gst.toFixed(2)));
        ...
        <div className="card radius-15 h-20">
          <div className="row">
            <div className="col-md-12">
              <center>
                <h2 className="mt-3">EDIT RECURRING BILL</h2>
              </center>
              <hr />
            </div>
          </div>
            ...
            <div className="card-body">
              <div id="salesOrder">
                ...
                <div className="row">
                  <div className="col-md-3 mt-3">
                    <div className="d-flex">
                      <label className="">Rec. Bill No.</label>
                      <span className="text-danger ml-3" id="BILLNoErr"></span>
                    </div>
                    <input
                      type="text"
                      className="form-control"
                  ...
                  <div className="col-md-3 mt-3">
                    <label className="">Credit Period </label>
                    <div className="d-flex align-items-center">
                      <select
                        className="form-control"
                        name="payment_term"
                ...
                <div className="row">
                  ...
                  <div className="col-md-5 mt-3 d-flex">
                    ...
                    <input
                      ...
                      onClick={() => navigate(`/view_rec_bill/${billId}/`)}
                      value="Cancel"
                      style={{ height: "fit-content" }}
                    />
                  </div>
                ...
                <span className="text-muted">
                  Reccurring Bill was created on a computer and is valid without
