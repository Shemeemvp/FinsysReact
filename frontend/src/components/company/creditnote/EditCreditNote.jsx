...
function EditCreditNote() {
  ...
  const fetchCreditNoteData = () => {
    axios
      .get(`${config.base_url}/fetch_credit_note_data/${ID}/`)
      .then((res) => {
        console.log("CN Data==", res);
        if (res.data.status) {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (res.data.status) {
          ...
          getInvoiceNumbersEdit(cNote.invoice_type, cNote.invoice_number, cNote.Customer);
  ...
  function getInvoiceNumbersEdit(type, number, customer){
    if (type != "" && number != "") {
      var i = {
        Id: ID,
        custId: customer,
      ...
      axios
        .get(`${config.base_url}/get_invoice_numbers_edit/`, { params: i })
        .then((res) => {
          if (res.data.status) {
            var inv = res.data.inv;
            setInvoiceNumbers([]);
            inv.map((i) => {
              setInvoiceNumbers((prevState) => [...prevState, i]);
            });
          } else {
            if (!res.data.status) {
              Swal.fire({
  ...
  function applyPriceListChange(val) {
    ...
    if (val) {
      ...
      document.getElementById("custPriceListName").style.display = "none";
      document.getElementById("custPriceListName").innerText = "";
      const updatedItems = creditNoteItems.map((item) => {
        return {
          ...item,
          priceListPrice: "",
        };
  ...
  async function applyPriceList(priceListId) {
    if (priceListId === "") {
      ...
      setPriceList(false);
      checkPriceList2();
      calc3(creditNoteItems);
    } else {
      let updatedItems = await Promise.all(
        creditNoteItems.map(async (pItem) => {
  ...
  const handleSubmit = (e) => {
    ...
    formData.append("credit_note_no", creditNoteNo);
    formData.append("credit_note_date", date);
    ...
    formData.append("creditNoteItems", JSON.stringify(creditNoteItems));
    ...
    axios
      .put(`${config.base_url}/update_credit_note/`, formData)
      .then((res) => {
        if (res.data.status) {
          Toast.fire({
            icon: "success",
          ...
          navigate(`/view_credit_note/${creditNoteId}/`);
        }
  ...
  function handleInvoiceTypeChange(val) {
    setInvoiceType(val);
    getInvoiceNumbers(val);
  }

  function getInvoiceNumbers(val) {
    var invType = val;
    ...
    if (customerId != "") {
      if (invType != "") {
        ...
        axios
          .get(`${config.base_url}/get_invoice_numbers/`, { params: i })
          .then((res) => {
            if (res.data.status) {
              var inv = res.data.inv;
              setInvoiceNumbers([]);
  ...
  function handleInvoiceNumberChange(val) {
    setInvoiceNumber(val);
    getInvoiceItems(val);
  }

  function getInvoiceItems(val) {
    var invType = invoiceType;
    var id = val;
    if (invType != "" && id != "") {
      var data = {
        Id: ID,
      ...
      axios
        .get(`${config.base_url}/get_invoice_items/`, { params: data })
        .then((res) => {
          setCreditNoteItems([]);
          if (res.data.status) {
            ...
            var inv = res.data.invItems;
            const cnItems = inv.map((i, index) => {
              if (i.item_type == "Goods") {
                var hsnSac = i.hsn;
              } else {
                var hsnSac = i.sac;
            ...
            if (!res.data.status) {
              ...
              setCreditNoteItems((prevState) => [...prevState, o]);
            }
          }
        })
        .catch((err) => {
      ...
      alert("Invoice Type and Number Should be selected.");
    }
  }
  ...
  function handleCreditNoteNoChange(val) {
    setCreditNoteNo(val);
    checkCreditNoteNo(val);
  }

  function checkCreditNoteNo(val) {
    document.getElementById("CNNoErr").innerText = "";
    var cnote_num = val;
    if (cnote_num != "") {
      var s = {
        Id: ID,
      ...
      axios
        .get(`${config.base_url}/check_credit_note_no/`, { params: s })
        .then((res) => {
          console.log("CN NUM Res=", res);
          if (!res.data.status) {
            document.getElementById("CNNoErr").innerText = res.data.message;
          } else {
            document.getElementById("CNNoErr").innerText = "";
          }
        })
  ...
  const handleCreditNoteItemsInputChange = (id, field, value) => {
    setCreditNoteItems((prevItems) =>
      prevItems.map((item) =>
  ...
  const handleItemChange = (value, id) => {
    ...
    if (!exists) {
      if (placeOfSupply != "") {
        handleCreditNoteItemsInputChange(id, "item", value);
      ...
      alert(
        "Item already exists in the Credit Note, choose another or change quantity.!"
      );
    }
  };
  ...
  const handleQuantityChange = (id, value) => {
    handleCreditNoteItemsInputChange(id, "quantity", value);
  ...
  function changeItemQty(id, value) {
    ...
    if (value != "") {
      if (parseInt(qty) > parseInt(avl_val)) {
        ...
        handleCreditNoteItemsInputChange(id, "quantity", parseInt(avl_val));
  ...
  function getItemData(item, id) {
    ...
    if (!exists) {
      if (plc != "") {
        if (priceList && PLId == "") {
          handleCreditNoteItemsInputChange(id, "item", "");
  ...
  const calc4 = (creditNoteItems, priceList) => {
    const updatedItems = creditNoteItems.map((item) => {
  ...
  function calc_total3(creditNoteItems) {
    var total = 0;
    var taxamount = 0;
  ...
  function calc_total(creditNoteItems) {
    var total = 0;
    var taxamount = 0;
    creditNoteItems.map((item) => {
      total += parseFloat(item.total || 0);
  ...
  function calc_total2(creditNoteItems, placeOfSupply) {
    var total = 0;
    var taxamount = 0;
    creditNoteItems.map((item) => {
      total += parseFloat(item.total || 0);
    ...
    <>
      ...
      >
        <div className="d-flex justify-content-end mb-1">
          <Link to={`/view_credit_note/${creditNoteId}/`}>
            <i
        ...
        <div className="card radius-15 h-20">
          <div className="row">
            <div className="col-md-12">
              <center>
                <h2 className="mt-3">EDIT CREDIT NOTE</h2>
              </center>
              <hr />
            </div>
          </div>
          ...
          <div className="card radius-15" style={{ minWidth: "100%" }}>
            <div className="card-body">
              <div id="salesOrder">
                ...
                <div className="row">
                  <div className="col-md-4 mt-3">
                    <div className="d-flex">
                      <label className="">Credit Note No.</label>
                      <span className="text-danger ml-3" id="CNNoErr"></span>
                    </div>
                    <input
                      type="text"
                      className="form-control"
                      ...
                      id="creditNoteNumber"
                      value={creditNoteNo}
                      onChange={(e) => handleCreditNoteNoChange(e.target.value)}
                      style={{ backgroundColor: "#43596c" }}
                      placeholder={nextCreditNoteNo}
                      required
                    />
                  </div>
                  <div className="col-md-4 mt-3">
                ...
                <div className="row">
                  <div className="col-md-4 mt-3">
                    <label className="">Credit Note Date:</label>
                    <input
                      type="date"
                      className="form-control"
                  ...
                  <div className="col-md-4 mt-3">
                    ...
                    >
                      ...
                      <option value="Recurring Invoice">
                        Recurring Invoice
                      </option>
                    </select>
                  </div>
                            ...
                            <tr key={row.id} id={`tab_row${row.id}`}>
                              ...
                              <td style={{ width: "13%" }}>
                                <select
                                  ...
                                  value={row.taxGst}
                                  onChange={(e) =>
                                    handleCreditNoteItemsInputChange(
                                      row.id,
                                      "taxGst",
                                      e.target.value
                                    )
                ...
                <div className="row">
                  ...
                  <div className="col-md-5 mt-3 d-flex">
                    ...
                    <input
                      ...
                      onClick={() => navigate(`/view_credit_note/${creditNoteId}/`)}
                      value="Cancel"
                      style={{ height: "fit-content" }}
                    />
                  </div>
                ...
                <span className="text-muted">
                  Credit Note was created on a computer and is valid without the
                  signature and seal.
                </span>
              </div>
            </div>
