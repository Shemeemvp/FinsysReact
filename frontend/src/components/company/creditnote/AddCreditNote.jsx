...
function AddCreditNote() {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (res.data.status) {
          ...
          setNextCreditNoteNo(res.data.cnNo);
        }
      })
      .catch((err) => {
        console.log(err);
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post(`${config.base_url}/create_new_credit_note/`, formData)
      .then((res) => {
  ...
  function checkItemQty(id, val){
    ...
    if(itmQty != "" && val != itmQty){
        document.getElementById('invQtyErr'+id).innerText='Item Quantity is changed.!'
    }else{
        ...
        <div className="card radius-15 h-20">
          <div className="row">
            <div className="col-md-12">
              <center>
                <h2 className="mt-3">NEW CREDIT NOTE</h2>
              </center>
              <hr />
            </div>
          </div>
                          ...
                          return (
                            <tr key={row.id} id={`tab_row${row.id}`}>
                              ...
                              <td style={{ width: "20%" }}>
                                ...
                                <input type="hidden" id={`invItemName${row.id}`} value={row.initialItem} />
                                <span id={`invNameErr${row.id}`} className="text-danger" style={{fontSize:"0.8rem",display:"block", width:"max-content"}}></span>
                              </td>
                              <td>
                                <input
                                  type="text"
                              ...
                              <td>
                                <input
                                  ...
                                  required
                                />
                                <input type="hidden" id={`invItemQty${row.id}`} value={row.initialQuantity} />
                                <span id={`invQtyErr${row.id}`} className="text-danger" style={{fontSize:"0.8rem",display:"block", width:"max-content"}}></span>
                              </td>
                              <td>
                                <input
                                  type="number"
