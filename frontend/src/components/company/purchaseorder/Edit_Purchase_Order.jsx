...
function Edit_Purchase_Order() {
  ...
  const [refNom, setRefNo] = useState("");
  ...
  const [CGst_type,setCGst_type] = useState([]);
  const [CGst_no,setCGst_no] = useState([]);
          ...
          .then((res) => {
            if (res.data.status) {
              ...
              var v = {
                ...
                label:purchase.vendor_name
              }
              var c= {
                value:purchase.Customer,
                label:purchase.customer_name
              }
              setVendorvalue(v)
              setCustomerValue(c)
              setVendor(purchase.Vendor);
              setEmail(purchase.vendor_email);
              setGstType(purchase.vendor_gst_type);
              setGstIn(purchase.vendor_gstin);
              setBillingAddress(purchase.vendor_address);
              setPlaceOfSupply(purchase.vendor_source_of_supply);
              setRefNo(purchase.reference_no);
              setSalesOrderNo(purchase.purchase_order_no);
              setDate(purchase.purchase_order_date);
              setShipmentDate(purchase.due_date);
              setTerm(purchase.payment_terms);
              setPaymentMethod(purchase.payment_method);
              setChequeNumber(purchase.cheque_no);
              setUpiId(purchase.upi_no);
              setAccountNumber(purchase.bank_acc_no);
              setCustomer(purchase.Customer);
              setCMail(purchase.customer_email);
              setCAddress(purchase.customer_address);
              setCGst_type(purchase.customer_gst_type);
              setCGst_no(purchase.customer_gstin);
              setCPlace(purchase.customer_place_of_supply);
              setSubTotal(purchase.subtotal);
              setIgst(purchase.igst);
              setCgst(purchase.cgst);
              setSgst(purchase.sgst);
              setTaxAmount(purchase.tax_amount);
              setShippingCharge(purchase.adjustment);
              setAdjustment(purchase.shipping_charge);
              setGrandTotal(purchase.grandtotal);
              setPaid(purchase.paid_off);
              setBalance(purchase.balance);
              setDescription(purchase.note);
              setpurchaseOrderItems([])
              const purchaseItems = itms.map((i)=>{
              ...
              refreshIndexes(purchaseItems);
              paymentMethodChange(purchase.payment_method);
              // checkTax(res.data.otherDetails.State,purchase.customer_place_of_supply);
              // checkPL(purchase.price_list_applied);
              // applyPriceList(purchase.price_list)
           }
          })
          .catch((err) => {
  ...
  const handleSubmit = (e) => {
    ...
    formData.append("reference_no", refNom);
  ...
  const [Vgst_type,setVGstType] = useState([]);
  const [Vgst_in,setVGstin] = useState([]);
  const [VBaddress,setVBaddress] = useState([]);
  const [VPlace,setVPlace] = useState([]);
      ...
      axios
        ...
        .then((res) => {
          if (res.data.status) {
            ...
            setVemail(vend.email);
            setVGstType(vend.gstType);
            setVGstin(vend.gstIn);
            setVBaddress(vend.address);
            setVPlace(vend.placeOfSupply);
            refreshTax(vend.placeOfSupply);
          }
        })
        .catch((err) => {
          console.log("ERROR", err);
  ...
  const update_purchase_order = (e) =>{
    ...
    axios
        .put(`${config.base_url}/Update_purchase_order/`, dt)
        .then((res) => {
          if (res.data.status) {
            Toast.fire({
              icon: "success",
  ...
  const [Vtitle, setVTitle] = useState("Mr");
    ...
    function handleVGST_TYPE(value) {
      setVeGstType(value);
      checkGstType(value);
    }
    ...
    return(
      ...
      >
        ...
        <div className="card radius-15 h-20">
          <div className="row">
            <div className="col-md-12">
              <center>
                <h2 className="mt-3">PURCHASE ORDER</h2>
              </center>
              <hr />
            </div>
          </div>
        ...
        <form
          ...
          onSubmit={update_purchase_order}
        >
          <div className="card radius-15" style={{minWidth:'100%'}}>
            ...
            <div className="row">
                  <div className="col-md-4 mt-2">
                    <label className="" style={{position:'relative',top:'10px'}}>Select Vendor</label>
                  ...
                  <div className="col-md-4 mt-3">
                    ...
                    <textarea
                      className="form-control"
                      name="vendor_bill_address"
                      id="vendorBillAddress"
                      rows="4"
                      style={{ backgroundColor: "#43596c", color: "white" }}
                  ...
                  {gstIn != "None" && (
                      <div
                        ...
                        id="vendorGstInDisplay"
                        style={{ display: "block",position:'relative',right:'15px',width:'480px' }}
                      >
            ...
            <hr/>
                <div className="row">
                  <div className="col-md-4 mt-3">
                    <div className="d-flex">
                      <label className="">Purchase Order No.</label>
                ...
                <div className="row">
                  ...
                  <div className="col-md-4 mt-2" >
                    <label className="" style={{position:'relative',top:'10px'}}>Payment Terms </label>
                    <div className="d-flex align-items-center">
                      <select
                        className="form-control"
                        name="payment_term"
            ...
            <hr/>
                ...
                <div className="row">
                  <div className="col-md-4 mt-2">
                    <label className="" style={{position:'relative',top:'10px'}}>Select Customer</label>
                    <span className="text-danger ml-3" id="custErr"></span>
                    <input
                      type="hidden"
                      name="customerId"
                ...
                <div className="row">
                  ...
                  <div className="col-md-5 mt-3 d-flex">
                    <input
                      ...
                      onClick={() =>navigate(`/edit_purchase_order/${id}`)}
                      value="Cancel"
                    />
                    <input
                      type="submit"
                ...
                <span className="text-muted">
                  Purchase Order was created on a computer and is valid without the
                  signature and seal.
                </span>
              </div>
            </div>
                ...
                <div className="card p-3">
                    <form method="post" id="newVendorForm" className="px-1">
                        ...
                        <div className="row mt-3">
                            <div className="col-md-3" style={{position:'relative',right:'20px'}}>
                                <label for="vendTitle">Title</label>
                                <select name="vendTitle" id="vendTitle" className="form-control" style={{backgroundColor:'#43596c',width:'240px'}} value={Vtitle} onChange={(e) => setVTitle(e.target.value)}>
                                    <option value="Mr">Mr</option>
                                    <option value="Mrs">Mrs</option>
                                    <option value="Ms">Ms</option>
                            ...
                            <div className="col-md-3" style={{position:'relative'}}>
                                ...
                                <input type="text" className="form-control" id="vendFirstName" name="first_name" required style={{backgroundColor:'#43596c',color:'white',width:'240px'}} value={VfirstName} onChange={(e) => setVFirstName(e.target.value)}/>
                                <div className="valid-feedback">Looks good!</div>
                            </div>
                        ...
                        <div className="row mt-3">
                            <div className="col-md-3" style={{position:'relative',right:'20px'}}>
                                ...
                                <input type="text" className="form-control" id="vendLocation" name="vendLocation" style={{backgroundColor:'#43596c',color:'white',width:'240px'}} value={Vlocation} onChange={(e) => setVLocation(e.target.value)}/>
                                <div className="valid-feedback">Looks good!</div>
                            </div>
                            <div className="col-md-3">
                                <label for="vendEmail">Email</label>
                                <input type="email" className="form-control" required id="vendEmail" name="email"  style={{backgroundColor:'#43596c',color:'white',width:'240px'}} placeholder="accuhub@gmail.com" value={Vmail} onChange={(e) => setVEmail(e.target.value)}/>
                                <div className="invalid-feedback">Please provide a valid Email</div>
                            </div>
                            <div className="col-md-3">
                                <label for="vendWebsite">Website</label>
                                <input type="text" className="form-control" id="vendWebsite" placeholder="www.accuhub.com"  name="website" style={{backgroundColor:'#43596c',color:'white'}} value={Vwebsite} onChange={(e) => handleWebSite(e.target.value)}/>
                        ...
                        <div className="row mt-3">
                            <div className="col-md-3">
                                ...
                                <select className="form-control" id="vendGstType" name="gst_type" style={{backgroundColor:'#43596c',color:'white'}} required value={VgstType} onChange={(e) => handleVGST_TYPE(e.target.value)}>
                                ...
                                <div className="invalid-feedback">Please select a valid registration type.</div>
                            </div>
                            ...
                            <div className="col-md-3">
                                ...
                                <input type="text" className="form-control" id="vendPanNo" name="pan_no" style={{backgroundColor:'#43596c',color:'white',width:'240px'}} required placeholder="APPCK7465F" value={VpanNo} onChange={(e) => handlePanNo(e.target.value)}/>
                                <div className="text-danger m-2" id="warnpan"></div>
                            </div>
                        ...
                        <div className="row">
                            <div className="col-md-3 mt-3">
                                ...
                                <select name="vendCurrency" id="vendCurrency" className="form-control" style={{backgroundColor:'#43596c',width:'245px'}} value={Vcurrency} onChange={(e) => setVCurrency(e.target.value)}>
                            ...
                            <div className="col-md-3 mt-3">
                                ...
                                <div className="d-flex">
                                    ...
                                    <input type="text" className="form-control" name="open_balance" id="vendopenbalance" step="any" style={{backgroundColor:'#43596c',color:'white',width:'145px'}} value={VoBal} onChange={(e) => setVOBal(e.target.value)}/>
                                    <div className="text-danger m-2" ></div>
                                </div>
                            </div>
                        ...
                        <div className="row mt-4">
                            <div className="col-4"></div>
                            <div className="col-4 d-flex justify-content-center">
                                <button className="btn btn-outline-secondary text-grey " type="submit" id="newVendorSave" style={{width:'100px'}} data-dismiss="modal" onClick={add_vendor_new}>Save</button>
                            </div>
                            <div className="col-4"></div>
                        </div>
                    </form>
