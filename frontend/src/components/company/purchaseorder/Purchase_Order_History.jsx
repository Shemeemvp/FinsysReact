...
function Purchase_order_History() {
    const ID = Cookies.get("Login_id");
    const { id } = useParams();
    const [history, setHistory] = useState([]);
    const [purchaseorder, setPurchaseOrder] = useState({});
  
    const fetchPurchaseOrderHistory = () => {
      axios
        .get(`${config.base_url}/fetch_purchase_order_history/${id}/`)
        .then((res) => {
          if (res.data.status) {
            var sls = res.data.purchaseOrder;
            var hist = res.data.history;
            setPurchaseOrder(sls);
            setHistory([]);
            hist.map((i) => {
            ...
            <div id="history">
              <center>
                <h3 className="mt-3 text-uppercase">
                  #{purchaseorder.purchase_order_no} - TRANSACTIONS
                </h3>
              </center>
...
export default Purchase_order_History;